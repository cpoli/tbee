...
class propagationTB():
    ...
    '''
    def __init__(self, coor, steps, dz):
        '''
        Plot the coordinates of  the Lieb lattice sites in hoppings space.

        :param coor: Sites coordinates. Output of the class **latticeTB**.
        :param sites: Number of sites. Output of the class **latticeTB**.
        :param steps: Number of steps.
        :param dz: Step.
        '''
        self.coor = coor
        self.sites = len(coor['tag'])
        self.steps = steps
        self.dz = dz
        self.prop = np.zeros((self.sites, self.steps), 'c16')
    ...
    def get_prop(self, ham, psi_init, norm=True):
        ...
        diag = 1j*np.ones(self.sites, 'c16')
    ...
    def plt_prop1d(self, s=300, fs=20):
        '''
        Get time evolution plot for 1D systems. 
        ...
        vmin, vmax = 0., np.max(color[:, 0: self.sites])
        
        extent = (0, self.steps*self.dz, -self.sites//2+0.5, self.sites-self.sites//2+0.5)
        aspect = 'auto'
    ...
    def prop_smooth1d(self, prop, a=14., no=40):
        ...
        smooth = np.empty((self.sites * no, self.steps))
    ...
    def get_ani(self, s=300, fs=20):
        ...
        scat = plt.scatter(self.coor['x'], self.coor['y'], c=color[:, 0],
