...
**lattice** creates finite one or two dimensional lattices. 
The set of available operations allow to build complex lattice structures.

**lattice**,

    * creates a generic lattice defined by:

        * position of the sites within the unit cell.

        * primitive vectors.

        * number of cells along the primitive vectors.

    * eliminate dangling sites (sites connected to just another site).

    * eliminate selected sites.

    * eliminate sites out of some boundaries (line or ellipse).

     * add or substact lattices.
